...
calc_daily_dose_baseline <- function(connector_or_df,
  ...
  if (filter_oral) {
    ...
    dc <- if ("drug_concept_name"  %in% names(drug_df))
      drug_df$drug_concept_name  else NULL
    ds <- if ("drug_source_value" %in% names(drug_df))
      drug_df$drug_source_value else NULL
